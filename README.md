...
- **Ingress:** Cloudflare Tunnel (Zero Trust) — two `cloudflared` replicas in `kube-system`, no inbound ports open on the router
...
The Kubernetes namespace is still named `nauka` ("learning") in the manifests. That name is left as-is — it records what actually ran on the cluster.
...
- **Provisioning was manual.** Proxmox VMs, the operating systems, and the K3s installation were all set up by hand. There is no Terraform, Ansible, cloud-init, or any other infrastructure automation — none, anywhere in this repository.
